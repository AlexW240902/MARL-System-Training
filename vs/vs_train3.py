...
class Args:
    ...
    track: bool = True                         ######## WANDB
    """if toggled, this experiment will be tracked with Weights and Biases"""
    wandb_project_name: str = "VS3_Train"
    ...
    ### DQN0
    learning_rate_d0: float = 2.5e-4         # The learning rate of the optimizer
    buffer_size0: int = 2000                 # The replay memory buffer size
    gamma_d0: float = 0.99                   # The discount factor gamma
    tau0: float = 0.01                       # The target network update rate
    ...
    end_e0: float = 0.05                     # The ending epsilon for exploration
    exploration_fraction0: float = 0.40      # The fraction of `total-timesteps` it takes from start-e to go end-e
    learning_starts0: int = 1000             # Timestep to start learning
    train_frequency0: int = 4                # The frequency of training

    ### PPO0
    learning_rate_p0: float = 2.5e-4         # The learning rate of the optimizer
    num_steps0: int = 128                    # Steps per policy rollout
    anneal_lr0: bool = True                  # Enable annealing for the learning rate
    gamma_p0: float = 0.99                   # Discount factor gamma
    gae_lambda0: float = 0.95                # Lambda for general advantage estimation
    num_minibatches0: int = 4                # Number of minibatches for every batch
    update_epochs0: int = 4                  # Number of times we train the model for every batch
    ...
    target_kl0: float = None                 # Maximum KL divergence allowed, if not set any
    batch_size_p0: int = None                # Size of a batch of information
    minibatch_size0: int = None              # Size of a minibatch of information


    ### DQN1
    learning_rate_d1: float = 2.5e-4         # The learning rate of the optimizer
    buffer_size1: int = 2000                 # The replay memory buffer size
    gamma_d1: float = 0.99                   # The discount factor gamma
    tau1: float = 0.005                      # The target network update rate
    ...
    end_e1: float = 0.05                     # The ending epsilon for exploration
    exploration_fraction1: float = 0.40      # The fraction of `total-timesteps` it takes from start-e to go end-e
    learning_starts1: int = 1000             # Timestep to start learning
    train_frequency1: int = 4                # The frequency of training

    ### PPO1
    learning_rate_p1: float = 2.5e-4         # The learning rate of the optimizer
    num_steps1: int = 128                    # Steps per policy rollout
    anneal_lr1: bool = True                  # Enable annealing for the learning rate
    gamma_p1: float = 0.99                   # Discount factor gamma
    gae_lambda1: float = 0.95                # Lambda for general advantage estimation
    num_minibatches1: int = 4                # Number of minibatches for every batch
    update_epochs1: int = 4                  # Number of times we train the model for every batch
    ...
    target_kl1: float = None                 # Maximum KL divergence allowed, if not set any
    batch_size_p1: int = None                # Size of a batch of information
    minibatch_size1: int = None              # Size of a minibatch of information
    ...
    if global_step > getattr(args, f'learning_starts{num_agent}'):
        if global_step % getattr(args, f'train_frequency{num_agent}') == 0:
            ...
            with torch.no_grad():
                ...
                getattr(args, f'gamma_d{num_agent}') * target_max * (1 - data.dones.flatten())
        ...
        if global_step % getattr(args, f'target_network_frequency{num_agent}') == 0:
            for target_network_param, q_network_param in zip(target_network.parameters(), q_network.parameters()):
                target_network_param.data.copy_(
                    getattr(args, f'tau{num_agent}') * q_network_param.data + \
                    (1.0 - getattr(args, f'tau{num_agent}')) * target_network_param.data
                )
...
def ppo_training(num_agent, global_step, agent, optimizer, next_obs, next_termination, \
next_truncation, rewards, terminations, truncations, values, obs, logprobs, actions):
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(getattr(args, f'num_steps{num_agent}'))):
            ...
            advantages[t] = lastgaelam = (
                delta + getattr(args, f'gamma_p{num_agent}') * getattr(args, \
                f'gae_lambda{num_agent}') * nextnonterminal * lastgaelam
            )
    ...
    for epoch in range(getattr(args, f'update_epochs{num_agent}')):
        ...
        for start in range(0, getattr(args, f'batch_size_p{num_agent}'), getattr(args, f'minibatch_size{num_agent}')):
            ...
            loss = pg_loss - getattr(args, f'ent_coef{num_agent}') * entropy_loss + v_loss * \
            getattr(args, f'vf_coef{num_agent}')
    ...
    # Writing down loss values and other metrics
    writer.add_scalar(
    ...
    for global_step in range(args.total_timesteps):
        ...
        if first_model == "dqn":
            ...
            if args.anneal_lr0:
                ...
                lrnow = frac * args.learning_rate_d0
            ...
            if args.anneal_lr0:
                frac = 1.0 - (global_step // args.batch_size_p0) / num_updates0
        ...
        if second_model == "dqn":
            ...
            if args.anneal_lr1:
                frac = (1.0 - global_step / args.total_timesteps)
                lrnow = frac * args.learning_rate_d1
            ...
            if args.anneal_lr1:
                frac = 1.0 - (global_step // args.batch_size_p1) / num_updates1
        ...
        # Join actions of both players
        actions = np.ravel(np.column_stack((step_actions0, step_actions1)))
        
        # Make an environment step (translate the masked actions to their actual number in the environment)
        ...
        if first_model == "dqn":
            ...
            if (global_step > 0) and (global_step % args.num_steps0 == 0):
                ppo_training(0, global_step, agent0, optimizer0, next_obs0, next_termination0, \
                next_truncation0, rewards0, terminations0, truncations0, values0, obs0, logprobs0, actions0)
        ...
        if second_model == "dqn":
            ...
            if (global_step > 0) and (global_step % args.num_steps1 == 0):
                ppo_training(1, global_step, agent1, optimizer1, next_obs1, next_termination1, next_truncation1,\
                rewards1, terminations1, truncations1, values1, obs1, logprobs1, actions1)
